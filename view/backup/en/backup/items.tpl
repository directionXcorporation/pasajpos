			...
			<md-dialog-content flex="noshrink">
				...
				<md-tabs md-dynamic-height md-border-bottom>
							...
							<form name="itemPrices" data-ng-disabled="formDisabled">
								<div layout-gt-xs="row" data-ng-repeat="itemPrice in privatedata.record.itemPrices">
									<md-input-container class="md-icon-float md-block" flex-gt-xs>
										...
										<input type="text" data-ng-model="itemPrice['itemsPrice_currencyId']" data-ng-change="itemPrice['itemsPrice_currencyId']=itemPrice['itemsPrice_currencyId'].toUpperCase();" currency-id />
									</md-input-container>
									<md-input-container class="md-icon-float md-block" flex-gt-xs>
										<label>{{_ITEM_PRICE}}</label>
										<input type="text" data-ng-model="itemPrice['itemsPrice_price']" price />
									</md-input-container>
					...
					<md-tab label="{{_ITEM_INVENTORY}}">
						<md-content class="md-padding">
							<form name="itemInventories" data-ng-disabled="formDisabled">
								<div layout-gt-xs="row" data-ng-repeat="itemInventory in privatedata.record.itemInventories">
									<md-input-container class="md-icon-float md-block" flex-gt-xs>
										<label>{{_STORE}}</label>
										<input type="text" data-ng-model="itemInventory['inventoryShotDetail_storeId']" />
									</md-input-container>
									<md-input-container class="md-icon-float md-block" flex-gt-xs>
										<label>{{_Quantity_on_hand}}</label>
										<input type="text" data-ng-model="itemInventory['inventoryShotDetail_quantityOnHand']" quantity/>
									</md-input-container>
									<md-input-container class="md-icon-float md-block" flex-gt-xs>
										<label>{{_Markdown_Reason}}</label>
										<input type="text" data-ng-model="itemInventory['inventoryShotHeader_markdownId']" />
									</md-input-container>
								</div>
								<md-button class="md-icon-button md-primary" aria-label="{{_ADD_INVENTORY}}" data-ng-click="addInventory(privatedata.record.itemInventories)">
									<md-icon class="material-icons">add_circle</md-icon>
								</md-button>
							</form>
						</md-content>
