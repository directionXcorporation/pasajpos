...
class usersModel{
	...
	*/
    ...
    public function addUsersPinCode($userId,$options=array()){
		/**
			* add User details to the database
			* $userId: Created uder id
			* $options {
	...
	public function searchFunctions($options=array()){
		/**
		...
		*/
		...
		$vals = array();
		$types = array();
		$sql = "SELECT function.functionCode AS functionCode,function.functionName AS functionName,function.functionDescription AS functionDescription,function.onlineInsertTime AS onlineInsertTime,function.updateTime AS updateTime,function.categoryCode AS categoryCode, function.dependsOnCode AS dependsOnCode, 
			functionsCategory.categoryName AS categoryName
			FROM function 
			INNER JOIN functionsCategory ON functionsCategory.categoryCode = function.categoryCode ";
		if(!empty(array_filter($searchCategory))>0){
		        $sql .= " OR function.categoryCodes = :categoryCodes ";
		        $vals[':categoryCodes'] = $searchCategory[0]['categoryCodes'];
		        $types[':categoryCodes'] = 's';
		        foreach($searchCategory as $key=> $searchCategoryElement){
		        	$sql .= " OR function.categoryCodes = :categoryCodes".$key;
		        	$vals[':categoryCodes'.$key] = $searchCategoryElement['categoryCodes'];
		        	$types[':categoryCodes'.$key] = 's';
		        }
		}
		if(!empty(array_filter($groupsId))>0){
			$sql .= " INNER JOIN usersFunctionAccess ON usersFunctionAccess.functionCode = function.functionCode 
				LEFT JOIN usersFunctionAccess AS usersFunctionAccess2 ON usersFunctionAccess.groupId = usersFunctionAccess2.groupId AND usersFunctionAccess2.onlineInsertTime GREATERTHAN usersFunctionAccess.onlineInsertTime AND usersFunctionAccess.functionCode = usersFunctionAccess2.functionCode AND usersFunctionAccess.userId = usersFunctionAccess2.userId ";
				
			$sql .= " WHERE usersFunctionAccess2.id IS NULL AND usersFunctionAccess.status =1  ";
			$sql .= " AND (usersFunctionAccess.groupId=:groupId ";
			$types[':groupId'] = 's';
			$vals[':groupId'] = $groupsId[0]['groupId'];
			foreach($groupsId as $key=>$groupsIdElement){
				$sql .= " OR usersFunctionAccess.groupId = :groupId".$key;
				$vals[':groupId'.$key] = $groupsIdElement['groupId'];
		        	$types[':groupId'.$key] = 's';
			}
			$sql .= ")";
		}
